...
export function POSView({ productos, onClose, onCreated }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background flex flex-col overflow-hidden">
      ...
      {/* ── Top bar ── */}
      <header className="shrink-0 flex items-center justify-between px-6 h-14 border-b border-border bg-card">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* RIGHT: cart panel */}
        <aside className="w-72 xl:w-80 shrink-0 hidden md:flex flex-col border-l border-border bg-card">
            ...
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-2 px-6 text-center text-muted-foreground">
                ...
                <p className="text-xs leading-relaxed">
                  Toca cualquier producto del panel izquierdo para agregarlo
                </p>
              </div>
            ) : (
                    ...
                    <li key={item.producto_id} className="px-5 py-3">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center border border-input rounded-lg overflow-hidden">
                          ...
                          <span className="w-8 text-center text-sm font-bold select-none">{item.cantidad}</span>
                          <button
                            onClick={() => updateQty(item.producto_id, 1)}
      ...
      {/* ── Mobile bottom bar ── */}
      {cart.length > 0 && (
        <div className="md:hidden shrink-0 p-3 border-t border-border bg-card shadow-[0_-4px_12px_rgba(0,0,0,0.08)]">
          <button
            onClick={handleCobrar}
            className="w-full flex items-center justify-between py-3 px-5 bg-primary text-primary-foreground rounded-xl font-bold active:opacity-90 transition-opacity min-h-[52px]"
          >
            <span className="bg-white/20 text-sm font-semibold px-2.5 py-1 rounded-full text-center min-w-[32px]">
              {totalItems}
            </span>
            <span className="text-base tracking-wide">Ver pedido</span>
